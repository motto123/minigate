...
func receiveMsgHandler(s *base.BusinessServer, sid uint64, msg *message.Message, ri *gate.ReceiveInfo) (reqMsg, respMsg proto.Message) {
	var err error
	req := new(chat.ReceiveReq)
	resp := &chat.ReceiveAck{CodeInfo: &common.ResponseCode{Code: common.Code_InternalError}}
	...
	if req.MsgId == 0 || req.SenderUid == 0 || req.ReceiverUid == 0 {
		resp.CodeInfo.Code = common.Code_InvalidParams
		log.Debugf(tag, "invalid params")
		return
	}
	...
	go func() {
		...
		newMsg := message.NewMessageAndNotCompressRoute(message.Notify, message.Protobuf, msg.Id, rabbitmq.RouteSendMsg, marshal,
		...
		err = s.PublishDataToWriteChanel(newMsg, &gate.ReceiveInfo{
			ReceiverUids: []int64{req.SenderUid},
			MsgId:        newMsg.Id,
